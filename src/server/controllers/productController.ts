...
export async function updateProduct(req: Request, res: Response) {
  try {
    ...
    const formattedUpdates = {
      ...
      regularPrice:
        updates.regularPrice !== undefined
          ? Number(updates.regularPrice)
          : undefined,
      salePrice:
        updates.salePrice !== undefined ? Number(updates.salePrice) : null,
    ...
    for (const field of numericFields) {
      if (
        formattedUpdates[field] !== undefined &&
        isNaN(formattedUpdates[field])
      ) {
        return res
          .status(400)
          .json({ error: `Invalid ${field}: must be a number` });
      }
    }

    // Validate image data if present
    if (
      formattedUpdates.image &&
      !formattedUpdates.image.startsWith("data:image")
    ) {
    ...
    if (formattedUpdates.additionalImages) {
      for (const img of formattedUpdates.additionalImages) {
        if (!img.startsWith("data:image")) {
          return res
            .status(400)
            .json({ error: "Invalid additional image format" });
        }
      }
    }
...
export async function getAllProducts(req: Request, res: Response) {
  try {
    ...
    if (brandQuery) {
      ...
      // First find the brand to make sure it exists
      // Filter products directly by brand ID
      products = products.filtered("brand == $0", brandQuery);
